...
def _():
    ...
    return NOTEBOOK_DIR, run_manim_scene, subprocess
...
def _(linear_render_btn, linear_scenes, mo, run_manim_scene):
    ...
    if linear_render_btn.value:
        ...
        video_data, error = run_manim_scene("linear_transformations", linear_scenes.value)
        if error:
            linear_output = mo.md(f"❌ {error}")
        elif video_data:
            linear_output = mo.video(src=video_data, controls=True, rounded=True)
        else:
...
def _(calculus_render_btn, calculus_scenes, mo, run_manim_scene):
    ...
    if calculus_render_btn.value:
        mo.output.append(mo.md("⏳ Rendering animation... this may take a moment."))
        video_data, error = run_manim_scene("calculus_visualizations", calculus_scenes.value)
        if error:
            calculus_output = mo.md(f"❌ {error}")
        elif video_data:
            calculus_output = mo.video(src=video_data, controls=True, rounded=True)
        else:
...
def _(mo, pythag_render_btn, pythag_scenes, run_manim_scene):
    ...
    if pythag_render_btn.value:
        mo.output.append(mo.md("⏳ Rendering animation... this may take a moment."))
        video_data, error = run_manim_scene("pythagorean_proofs", pythag_scenes.value)
        if error:
            pythag_output = mo.md(f"❌ {error}")
        elif video_data:
            pythag_output = mo.video(src=video_data, controls=True, rounded=True)
        else:
